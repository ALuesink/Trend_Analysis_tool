        ...
        try:        
            ...
            if run in runs_db:
                ...
                for lane in lane_dict:
                    insert_Lane = Run_per_Lane.insert().values(Lane=str(lane_dict[lane]['Lane']), PF_Clusters=lane_dict[lane]['PF_Clusters'], PCT_of_lane=lane_dict[lane]['PCT_of_lane'], PCT_perfect_barcode=lane_dict[lane]['PCT_perfect_barcode'], PCT_one_mismatch_barcode=lane_dict[lane]['PCT_one_mismatch_barcode'], Yield_Mbases=lane_dict[lane]['Yield_Mbases'], PCT_PF_Clusters=lane_dict[lane]['PCT_PF_Clusters'], PCT_Q30_bases=lane_dict[lane]['PCT_Q30_bases'], Mean_Quality_Score=lane_dict[lane]['Mean_Quality_Score'], Run_ID=run_ID)
                    conn.execute(insert_Lane)
                    
                for sample in samples_dict:
                    insert_Sample = Sample_Sequencer.insert().values(Lane=str(samples_dict[sample]['Lane']),Project=samples_dict[sample]['Project'].upper(),Sample_name=samples_dict[sample]['Sample_name'],Barcode_sequence=samples_dict[sample]['Barcode_sequence'],PF_Clusters=samples_dict[sample]['PF_Clusters'],PCT_of_lane=samples_dict[sample]['PCT_of_lane'],PCT_perfect_barcode=samples_dict[sample]['PCT_perfect_barcode'],PCT_one_mismatch_barcode=samples_dict[sample]['PCT_one_mismatch_barcode'],Yield_Mbases=samples_dict[sample]['Yield_Mbases'],PCT_PF_Clusters=samples_dict[sample]['PCT_PF_Clusters'],PCT_Q30_bases=samples_dict[sample]['PCT_Q30_bases'],Mean_Quality_Score=samples_dict[sample]['Mean_Quality_Score'],Run_ID=run_ID)
